...
|---|----------|----------------------|-----------|-------------|
| 0 | **Structural review** | Restructure before auditing. God files impede review. | `sys_process.c` (2059 LOC), `sys_file.c` (1194), `vfs.c` | 0 (manual) |
...
| 7 | **Signal & process lifecycle** | Signals delivered via SYSRET path corrupt callee-saved registers (C5 — known TODO). Fork/exec/exit race conditions. Orphan handling. PID reuse. | `kernel/syscall/sys_signal.c`, `sys_process.c` (2K LOC), `signal.c` | 2 |
