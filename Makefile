...
user/shutdown/shutdown.elf: $(MUSL_BUILT)
	$(MAKE) -C user/shutdown

user/reboot/reboot.elf: $(MUSL_BUILT)
	$(MAKE) -C user/reboot
...
DISK_USER_BINS = \
	...
	user/shutdown/shutdown.elf \
	user/reboot/reboot.elf \
	build/curl/curl \
	build/musl-dynamic/usr/lib/libc.so
